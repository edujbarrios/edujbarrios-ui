export const componentCode = {
  "model-route-simulator": `const routes = [{ condition: "Simple lookup", model: "Fast", cost: "$0.002" }, { condition: "Multi-step reasoning", model: "Deep", cost: "$0.041" }, { condition: "Sensitive action", model: "Deep + review", cost: "$0.058" }]; export function ModelRouteSimulator() { return <ul>{routes.map((route) => <li key={route.condition}>{route.condition}: {route.model} · {route.cost}</li>)}</ul>; }`,
  "synthetic-user-cohort": `const personas = [{ name: "Power user", cases: 48, pass: 94 }, { name: "First-time visitor", cases: 72, pass: 86 }, { name: "Ambiguous requester", cases: 35, pass: 69 }]; export function SyntheticUserCohort() { return <ul>{personas.map((persona) => <li key={persona.name}>{persona.name}: {persona.pass}% across {persona.cases} cases</li>)}</ul>; }`,
  "guardrail-decision-trace": `const decisions = [{ rule: "PII detection", result: "Redact" }, { rule: "Allowed topics", result: "Pass" }, { rule: "Action scope", result: "Review" }]; export function GuardrailDecisionTrace() { return <ol>{decisions.map((decision) => <li key={decision.rule}>{decision.rule}: {decision.result}</li>)}</ol>; }`,
  "response-repair-queue": `const repairs = [{ issue: "Malformed JSON", strategy: "Schema retry", state: "Repairing" }, { issue: "Missing citation", strategy: "Targeted retrieval", state: "Queued" }, { issue: "Invalid date", strategy: "Field correction", state: "Ready" }]; export function ResponseRepairQueue() { return <ol>{repairs.map((repair) => <li key={repair.issue}>{repair.issue}: {repair.strategy} · {repair.state}</li>)}</ol>; }`,
  "citation-claim-matrix": `const claims = [{ claim: "Revenue grew 18%", sources: 2, status: "Supported" }, { claim: "Expansion led growth", sources: 1, status: "Partial" }, { claim: "Churn fell in Europe", sources: 0, status: "Missing" }]; export function CitationClaimMatrix() { return <ul>{claims.map((item) => <li key={item.claim}>{item.claim}: {item.sources} sources · {item.status}</li>)}</ul>; }`,
  "evaluation-regression-timeline": `const releases = [{ version: "v21", score: 94 }, { version: "v22", score: 92 }, { version: "v23", score: 81 }, { version: "v24", score: 89 }]; export function EvaluationRegressionTimeline() { return <ol>{releases.map((release) => <li key={release.version}>{release.version}: {release.score}</li>)}</ol>; }`,
  "ai-incident-triage-board": `const events = [{ time: "09:42", label: "Citation rate dropped" }, { time: "09:38", label: "Retriever index changed" }, { time: "09:31", label: "Eval threshold breached" }]; export function AiIncidentTriageBoard() { return <ol>{events.map((event) => <li key={event.time}>{event.time}: {event.label}</li>)}</ol>; }`,
  "prompt-injection-quarantine": `const signals = ["Hidden instruction in retrieved HTML", "Request to reveal system context"]; export function PromptInjectionQuarantine() { return <section><h3>Prompt injection quarantined</h3><ul>{signals.map((signal) => <li key={signal}>{signal}</li>)}</ul></section>; }`,
  "handoff-readiness-card": `const checks = [{ label: "Decision summary", detail: "Generated", ready: true }, { label: "Supporting evidence", detail: "4 sources attached", ready: true }, { label: "Open risks", detail: "1 needs review", ready: false }, { label: "Next owner", detail: "Support lead", ready: true }]; export function HandoffReadinessCard() { return <ul>{checks.map((check) => <li key={check.label}>{check.label}: {check.detail}</li>)}</ul>; }`,
  "output-schema-drift-monitor": `const fields = [{ name: "confidence", change: "number → string", impact: "Breaking" }, { name: "citations[]", change: "optional → required", impact: "Breaking" }, { name: "trace_id", change: "new optional field", impact: "Safe" }]; export function OutputSchemaDriftMonitor() { return <ul>{fields.map((field) => <li key={field.name}>{field.name}: {field.change} · {field.impact}</li>)}</ul>; }`,
  "context-conflict-resolver": `const claims = [{ source: "Pricing guide · v8", claim: "Refund window: 30 days", status: "Current" }, { source: "Support article · 2024", claim: "Refund window: 14 days", status: "Stale" }]; export function ContextConflictResolver() { return <ul>{claims.map((item) => <li key={item.source}>{item.source}: {item.claim} · {item.status}</li>)}</ul>; }`,
  "tool-permission-inspector": `const permissions = [{ tool: "Knowledge search", scope: "Read public docs", status: "Allowed" }, { tool: "Customer records", scope: "Read contact data", status: "Approval" }, { tool: "Email sender", scope: "Send as workspace", status: "Blocked" }]; export function ToolPermissionInspector() { return <ul>{permissions.map((item) => <li key={item.tool}>{item.tool}: {item.scope} · {item.status}</li>)}</ul>; }`,
  "token-budget-allocator": `const allocations = [{ label: "System", tokens: "4.2k" }, { label: "History", tokens: "9.8k" }, { label: "Retrieval", tokens: "5.1k" }, { label: "Response", tokens: "4.8k" }]; export function TokenBudgetAllocator() { return <ul>{allocations.map((item) => <li key={item.label}>{item.label}: {item.tokens}</li>)}</ul>; }`,
  "retrieval-freshness-gauge": `const sources = [{ label: "Product docs", age: "2h", freshness: 96 }, { label: "Support index", age: "3d", freshness: 78 }, { label: "Policy archive", age: "41d", freshness: 32 }]; export function RetrievalFreshnessGauge() { return <ul>{sources.map((source) => <li key={source.label}>{source.label}: updated {source.age} ago</li>)}</ul>; }`,
  "dataset-slice-explorer": `const slices = [{ label: "English · Enterprise", score: 96 }, { label: "Spanish · SMB", score: 82 }, { label: "Complex queries", score: 74 }]; export function DatasetSliceExplorer() { return <ul>{slices.map((slice) => <li key={slice.label}>{slice.label}: {slice.score}%</li>)}</ul>; }`,
  "tool-latency-waterfall": `const calls = [{ name: "Search", time: "420ms" }, { name: "Database", time: "710ms" }]; export function ToolLatencyWaterfall() { return <ol>{calls.map((call) => <li key={call.name}>{call.name}: {call.time}</li>)}</ol>; }`,
  "hallucination-risk-banner": `const risks = ["2 claims lack citations", "Source freshness exceeds 90 days"]; export function HallucinationRiskBanner() { return <section><h3>Elevated hallucination risk</h3><ul>{risks.map((risk) => <li key={risk}>{risk}</li>)}</ul></section>; }`,
  "human-override-ledger": `const overrides = [{ person: "M. Chen", change: "Blocked → Approved" }, { person: "A. Silva", change: "Auto → Review" }]; export function HumanOverrideLedger() { return <ol>{overrides.map((item) => <li key={item.person + item.change}>{item.change} by {item.person}</li>)}</ol>; }`,
  "context-compression-preview": `const sections = [{ label: "Decisions", action: "Keep" }, { label: "Discussion", action: "Summarize" }, { label: "Duplicates", action: "Drop" }]; export function ContextCompressionPreview() { return <ul>{sections.map((section) => <li key={section.label}>{section.label}: {section.action}</li>)}</ul>; }`,
...
export function TokenStreamTimeline() {
  ...
  "adversarial-test-card": `const attacks = [{ name: "Instruction override", result: "Blocked" }, { name: "Secret extraction", result: "Blocked" }, { name: "Role confusion", result: "Review" }];
...
export function SpeculativeModelRace() {
  ...
  "context-provenance-map": `const sources = [{ name: "System policy", tokens: "1.2k", influence: "High" }, { name: "Product brief.pdf", tokens: "3.8k", influence: "High" }, { name: "User message", tokens: "184", influence: "Direct" }];
...
export function ContextProvenanceMap() {
  ...
  "adversarial-prompt-lab": `const attacks = [{ name: "Instruction override", result: "Blocked" }, { name: "Data extraction", result: "Blocked" }, { name: "Role confusion", result: "Review" }];
...
export function AutonomousRunBudget() {
  ...
  "agent-activity-feed": `const events = [
...
export function AgentActivityFeed() {
  ...
  "mcp-connection-card": `const capabilities = ["Search docs", "Read resources", "Run tools"];
...
export function McpConnectionCard() {
  ...
  "prompt-quality-score": `const checks = [
...
export function PromptQualityScore() {
  ...
  "answer-evidence-card": `const evidence = [
...
export function AnswerEvidenceCard() {
  return <section className="w-full max-w-md rounded-xl border border-white/12 bg-[#0b0f14]/92 p-4 shadow-2xl">
    <header className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#40E0D0]">Answer evidence</p><h3 className="mt-1 text-base font-bold text-white">3 claims supported</h3></div><span className="rounded-full bg-[#40E0D0]/10 px-2.5 py-1 text-[0.65rem] font-bold text-[#d8fffb]">Verified</span></header>
    <ul className="mt-4 space-y-2">{evidence.map((item) => <li key={item.claim} className="grid grid-cols-[1fr_auto] gap-3 rounded-lg border border-white/9 bg-white/[0.025] p-3"><div><p className="text-xs font-semibold text-slate-200">{item.claim}</p><p className="mt-1 text-[0.65rem] text-slate-500">{item.source}</p></div><span className="self-center text-xs font-black text-white" aria-label={item.confidence + "% confidence"}>{item.confidence}%</span></li>)}</ul>
  ...
  "model-availability-grid": `const models = [
...
export function ModelAvailabilityGrid() {
  ...
  "tool-cost-breakdown": `const tools = [
...
export function ToolCostBreakdown() {
  ...
  "locale-coverage-card": `const locales = [{ name: "English", code: "en-US", coverage: 100 }, { name: "Spanish", code: "es-ES", coverage: 92 }, { name: "Japanese", code: "ja-JP", coverage: 68 }];
...
export function LocaleCoverageCard() {
  ...
  "quota-forecast-card": `const usage = [42, 48, 46, 58, 64, 71, 78];
...
export function QuotaForecastCard() {
  return <section className="w-full max-w-sm rounded-xl border border-white/12 bg-[#0b0f14]/92 p-4 shadow-2xl">
    ...
    <div className="mt-5 flex h-24 items-end gap-2" aria-label="Seven day quota usage trend">{usage.map((value, index) => <div key={index} className="flex flex-1 flex-col items-center gap-2"><span className="w-full rounded-t bg-gradient-to-t from-[#a78bfa] to-[#40E0D0]" style={{ height: value + "%" }} /><span className="text-[0.6rem] text-slate-600">{index + 1}</span></div>)}</div>
